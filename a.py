    ...
    if len(detections) > 0:  						# At least 1 detection in the image and check detection presence in a frame  
        ...
        for label, confidence, bbox in detections:				# In this if statement, we filter all the detections for persons only
            # Check for the only person name tag 
              # Coco file has string of all the names
            if label == 'person':                
                 	# Store the center points of the detections
                x,y,w,h=bbox
                xmin, ymin, xmax, ymax = bbox2points(bbox)   # Convert from center coordinates to rectangular coordinates, We use floats to ensure the precision of the BBox            
...
def YOLO():
    ...
    cap = cv2.VideoCapture('http://192.168.0.100:4747/video')       # Uncomment for Video from Mobile Camera (DroidCam Hosted Camera)
...
if __name__ == "__main__":
>>>>>>> 7906e01d5701e1e48d8614b0f9e212c49d40ad87
    YOLO()                            